...
class GithubEntityProvider {
  ...
  async getOrganizations() {
    ...
    const githubAppMux = new integration.GithubAppCredentialsMux(this.integration, [
      this.config.app
    ]);
    const installs = await githubAppMux.getAllInstallations();
    return installs.map(
      (install) => install.target_type === "Organization" && install.account && "login" in install.account ? install.account.login : void 0
    ).filter(Boolean);
  }
  async createGraphqlClient(organization) {
  ...
  matchesFilters(repositories) {
    ...
    return repositories.filter((r) => {
      ...
      return (allowArchived || !r.isArchived) && (!repositoryFilter || repositoryFilter.test(r.name)) && util.satisfiesTopicFilter(repoTopics, topicFilters) && util.satisfiesForkFilter(allowForks, r.isFork) && util.satisfiesVisibilityFilter(visibilities, r.visibility) && r.defaultBranchRef;
    });
  }
  ...
  async onPush(event) {
    const configOrganizations = (await this.getOrganizations()).map(
      (org) => org.toLocaleLowerCase("en-US")
    );
    const eventOrganization = event.organization?.login.toLocaleLowerCase("en-US");
    if (!eventOrganization || !configOrganizations.includes(eventOrganization)) {
      this.logger.debug(
  ...
  async addEntitiesForRepo(repository) {
    if (this.config.validateLocationsExist) {
      ...
      const repositoryFromGithub = await github.getOrganizationRepository(
        ...
        (r) => r ? this.createRepoFromGithubResponse(r, organization) : null
      );
  ...
  createRepoFromGithubResponse(repositoryResponse, organization) {
    return {
